...
test-api:
	...
	docker run --network calc-test-api --volume `pwd`:/opt/calc/results --name api-tests --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest --junit-xml=results/api_result.xml -m api  || true
	docker run --rm --volume `pwd`:/opt/calc/results --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest junit2html results/api_result.xml results/api_result.html
...
test-e2e:
	...
	docker run -d --network calc-test-e2e --volume `pwd`:/opt/calc/results --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	docker run -d --network calc-test-e2e --name calc-web -p 80:80 calc-web
	docker create --network calc-test-e2e --volume `pwd`:/results --name e2e-tests cypress/included:4.9.0 --browser chrome || true
	...
	docker run --rm --volume `pwd`:/opt/calc/results --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest junit2html results/cypress_result.xml results/cypress_result.html
	docker cp apiserver:/opt/calc/results ./  || true
	docker rm --force apiserver  || true
	docker rm --force calc-web || true
	docker rm --force e2e-tests || true
