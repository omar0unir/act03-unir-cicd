...
test-unit:
	...
	docker rm --force unit-tests2 || true
	...
	docker run --name unit-tests2 --env PYTHONPATH=/opt/calc -w /opt/calc -v `pwd`/results:/opt/calc/results calculator-app:latest junit2html results/unit_result.xml results/unit_result.html
	docker cp unit-tests2:/opt/calc/results ./
	docker rm unit-tests || true
	docker rm unit-tests2 || true
...
test-api:
	...
	docker run --network calc-test-api --name api-tests --volume `pwd`:/opt/calc --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest --junit-xml=results/api_result.xml -m api  || true
	docker run --network calc-test-api --rm --volume `pwd`:/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest junit2html results/api_result.xml results/api_result.html
...
test-e2e:
	...
	docker run -d --volume `pwd`:/opt/calc --network calc-test-e2e --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	...
	docker cp apiserver:/results ./  || true
	docker rm --force apiserver  || true
	docker rm --force calc-web || true
	docker rm --force e2e-tests || true
